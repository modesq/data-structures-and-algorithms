...
const validatePin = (pin) => {
  let regex = /\d/g;
  let x = pin.toString().match(regex);
  try {
    if (x.length === 4) {
      return true;
    }
...
const validateWord = (word) => {
  // Solution code here...
  var regex = /[a-z]/ig;
  var count = word.toString().match(regex);
  try {
    if (count.length >= 5 && count.length <= 10) {
      return true;
    } else {
      return false;
    }
...
const hasNumber = (string) => {
  // Solution code here...
  var regex = /[A-Za-z]\d+/;
  var countNums = regex.test(string);
  return countNums;
};
...
const validateEmail = (email) => {
  // Solution code here...
  let regex = /^[A-Za-z0-9]+(\.[A-Za-z0-9]+)?@[a-z]*(\.com|\.net|\.org)$/;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let reg = /^(\([0-9]{3,3}\)|[0-9]{3,3})[\- ]?[0-9]{3,3}[\- ]?[0-9]{4,4}$/;
  let phone = reg.test(phoneNumber);
  return phone;
};
/* ------------------------------------------------------------------------------------------------
TESTS
