...
let biggerThanLuke = (arr) => {
  const largerThanLuke = [];
  let lukesMass = 0;
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].name === "Luke Skywalker") {
      lukesMass = arr[i].mass;
    }
    if (parseInt(arr[i].mass) > parseInt(lukesMass)) {
      largerThanLuke.push(arr[i].name);
    }
  }
  return (largerThanLuke.join(' - '));
};
